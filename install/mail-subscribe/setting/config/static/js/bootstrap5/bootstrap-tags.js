	...
	TagsInput.prototype = {
		...
		build : function(options) {
			...
			if (self.options.typeaheadjs) {
				...
				// Determine if main configurations were passed or simply a
				// dataset
			...
			if (self.options.addOnBlur && self.options.freeInput) {
				self.$input.on('focusout', $.proxy(
						function(event) {
							// HACK: only process on focusout when no typeahead
							// opened, to
									...
									.proxy(
											function(event) {
												...
												if (self.options.freeInput
													...
													// Only attempt to add a tag
													// if there is data in the
													// field
													if (text.length !== 0) {
														self
																.add(maxLengthReached
																		? text
																				.substr(
																						0,
